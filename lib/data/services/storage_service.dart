...
class StorageService {
  ...
  Future<String?> uploadProfilePhoto(String uid, Uint8List bytes, String filename) async {
    ...
    await _client.storage.from(Env.storageBucketProfile).uploadBinary(path, bytes, fileOptions: const FileOptions(upsert: true));
  ...
  Future<String?> uploadPostImage(String uid, Uint8List bytes, String filename) async {
    final path = '$uid/$filename';
    await _client.storage.from(Env.storageBucketPost).uploadBinary(path, bytes, fileOptions: const FileOptions(upsert: true));
